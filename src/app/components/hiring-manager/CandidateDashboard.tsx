...
export function CandidateDashboard({ jobs, candidates }: Props) {
  const [prestigeNeutralizer, setPrestigeNeutralizer] = useState(false);
  const [anonymizedMode, setAnonymizedMode] = useState(false);

  const activePositions = jobs.length;
  ...
  const neutralizeText = (text: string): string => {
    if (!prestigeNeutralizer) return text;
    const replacements = [
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white border border-[#e4e1da] p-4 rounded-xl shadow-lg">
          <p className="text-sm text-[#1c1c1a] mb-2">{data.name}</p>
  ...
  const kpiCards = [
    { label: 'Active Positions', value: activePositions, icon: Briefcase, color: '#2d6a55' },
    { label: 'Total Candidates', value: totalCandidates, icon: Users, color: '#2d6a55' },
    { label: 'Screening Completed', value: screeningCompleted, icon: CheckCircle2, color: '#2d6a55' },
  ];
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-[#1c1c1a]">Candidate Pipeline</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {kpiCards.map(({ label, value, icon: Icon }, idx) => (
          ...
          >
            ...
            <p className="text-3xl text-[#1c1c1a] mb-1">{value}</p>
      ...
      <motion.div
        ...
        className="bg-white border border-[#e4e1da] rounded-2xl p-6"
      >
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-[#1c1c1a]">Bias Mitigation Controls</h3>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between p-4 bg-[#f7f6f3] rounded-xl border border-[#e4e1da] hover:border-[#2d6a55]/30 transition-colors">
            <div className="flex-1 mr-6">
              <div className="flex items-center gap-2 mb-0.5">
                <label htmlFor="prestige-toggle" className="text-sm text-[#1c1c1a] cursor-pointer">
                  Prestige Neutralizer
                </label>
                <span className={`px-2 py-0.5 rounded-full text-xs transition-colors ${
                  prestigeNeutralizer ? 'bg-[#e8f2ee] text-[#2d6a55]' : 'bg-[#f0ede8] text-[#a8a49d]'
                }`}>
                  {prestigeNeutralizer ? 'Active' : 'Off'}
                </span>
              </div>
              <p className="text-xs text-[#6b7063]">
                Replace company and university names with descriptive categories
              </p>
            </div>
            <Switch.Root
              id="prestige-toggle"
              checked={prestigeNeutralizer}
              onCheckedChange={setPrestigeNeutralizer}
          ...
          <div className="flex items-center justify-between p-4 bg-[#f7f6f3] rounded-xl border border-[#e4e1da] hover:border-[#2d6a55]/30 transition-colors">
            <div className="flex-1 mr-6">
              <div className="flex items-center gap-2 mb-0.5">
                <label htmlFor="anonymous-toggle" className="text-sm text-[#1c1c1a] cursor-pointer">
                  Anonymized Blind Hiring
                </label>
                <span className={`px-2 py-0.5 rounded-full text-xs transition-colors ${
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-[#1c1c1a]">Trajectory Analysis</h3>
              ...
              <ResponsiveContainer width="100%" height={360}>
                <ScatterChart margin={{ top: 20, right: 20, bottom: 30, left: 20 }}>
                  ...
                  <XAxis
                    ...
                    unit="%"
                    domain={[0, 100]}
                    label={{ value: 'Core Match Fit (%)', position: 'insideBottom', offset: -12, style: { fill: '#6b7063', fontSize: 12 } }}
                    stroke="#e4e1da"
                  ...
                  <YAxis
                    ...
                    unit="%"
                    domain={[0, 100]}
                    label={{ value: 'Trajectory (Learning Rate)', angle: -90, position: 'insideLeft', style: { fill: '#6b7063', fontSize: 12 } }}
                    stroke="#e4e1da"
                    tick={{ fill: '#a8a49d', fontSize: 11 }}
                  />
                  <ZAxis range={[80, 300]} />
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className="bg-[#f0f9f4] border border-[#c8e6d8] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <p className="text-xs text-[#2d6a55] uppercase tracking-wider">High Match + High Trajectory</p>
                </div>
              ...
              <div className="bg-[#fdf8ee] border border-[#e8d8a0] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <p className="text-xs text-[#c9a84c] uppercase tracking-wider">Hidden Gems</p>
                </div>
      ...
      >
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-[#1c1c1a]">Active Pipeline</h3>
            <p className="text-xs text-[#6b7063]">
        ...
        {sortedCandidates.length === 0 ? (
          <div className="text-center py-14">
            ...
            <p className="text-sm text-[#1c1c1a] mb-1">No candidates in pipeline yet</p>
              ...
              return (
                ...
                >
                  <Accordion.Item
                    ...
                    className="border border-[#e4e1da] rounded-xl overflow-hidden hover:border-[#2d6a55]/30 transition-colors"
                  >
                    <Accordion.Header>
                      <Accordion.Trigger className="w-full px-5 py-4 bg-[#f7f6f3] hover:bg-[#f0ede8] transition-colors text-left group">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4 flex-1">
                            <div className="relative flex-shrink-0">
                              <div className="w-12 h-12 bg-[#e8f2ee] rounded-xl flex items-center justify-center text-[#2d6a55]">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-0.5">
                                <h4 className="text-sm text-[#1c1c1a]">{displayName}</h4>
                              ...
                              <p className="text-xs text-[#a8a49d] mt-0.5">{job?.title}</p>
                            </div>
                            ...
                            <div className="flex items-center gap-5 mr-4">
                              ...
                              <div className="text-center">
                                <div className="text-lg text-[#c9a84c]">{candidate.trajectoryScore}</div>
                    ...
                    <Accordion.Content className="px-5 py-5 border-t border-[#e4e1da] bg-white overflow-hidden">
                      <div className="space-y-5">
                        ...
                        <div className="bg-[#f7f6f3] border border-[#e4e1da] rounded-xl p-4">
                          <p className="text-xs tracking-wider uppercase text-[#a8a49d] mb-2">Why This Person?</p>
                          <p className="text-sm text-[#6b7063] leading-relaxed italic">
                            "Exceptional trajectory — moving from mid-level to senior in record time. Strong architectural thinking combined with hands-on execution. A rare find who balances technical depth with leadership potential."
                          </p>
                        </div>

                        {/* Profile */}
                        <div className="bg-white border border-[#e4e1da] rounded-xl p-4">
                          <p className="text-xs tracking-wider uppercase text-[#a8a49d] mb-3">Profile</p>
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                              <span className="text-xs text-[#a8a49d]">Email</span>
                              <p className="text-[#1c1c1a] mt-0.5">{displayEmail}</p>
                            </div>
                            <div>
                              <span className="text-xs text-[#a8a49d]">Location</span>
                              <p className="text-[#1c1c1a] mt-0.5">{candidate.location}</p>
                            </div>
                          </div>
                          {candidate.about && (
                        ...
                        {candidate.experiences?.length > 0 && (
                          <div className="bg-white border border-[#e4e1da] rounded-xl p-4">
                            <p className="text-xs tracking-wider uppercase text-[#a8a49d] mb-3">Experience</p>
                            <div className="space-y-3">
                              {candidate.experiences.map((exp, idx) => (
                                <div key={idx} className="flex gap-3">
                                  <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-[#2d6a55] flex-shrink-0" />
                                  <div>
                                    <p className="text-sm text-[#1c1c1a]">{exp.title}</p>
                                    <p className="text-xs text-[#6b7063]">{neutralizeText(exp.company)}</p>
                        ...
                        {candidate.education?.length > 0 && (
                          <div className="bg-white border border-[#e4e1da] rounded-xl p-4">
                            <p className="text-xs tracking-wider uppercase text-[#a8a49d] mb-3">Education</p>
                            <div className="space-y-2">
                              {candidate.education.map((edu, idx) => (
                                <div key={idx}>
                                  <p className="text-sm text-[#1c1c1a]">{neutralizeText(edu.school)}</p>
                        ...
                        {/* AI Committee Debate */}
                        <div>
                          <p className="text-xs tracking-wider uppercase text-[#a8a49d] mb-3">AI Committee Analysis</p>
                          <div className="grid md:grid-cols-2 gap-3">
                            <div className="bg-[#f0f9f4] border border-[#c8e6d8] rounded-xl p-4">
                              <div className="flex items-center gap-2 mb-3">
                                <TrendingUp className="w-4 h-4 text-[#2d6a55]" />
                                <p className="text-xs text-[#2d6a55] uppercase tracking-wider">Talent Advocate</p>
                              </div>
                              {candidate.advocatePros?.length ? (
                                <ul className="space-y-2">
                                  {candidate.advocatePros.map((pro, idx) => (
                                    <li key={idx} className="text-xs text-[#3d5a4a] flex items-start gap-2 leading-relaxed">
                            ...
                            <div className="bg-[#fdf8ee] border border-[#e8d8a0] rounded-xl p-4">
                              <div className="flex items-center gap-2 mb-3">
                                ...
                                <p className="text-xs text-[#c9a84c] uppercase tracking-wider">Critical Recruiter</p>
                              </div>
                              {candidate.recruiterCons?.length ? (
                                <ul className="space-y-2">
                                  {candidate.recruiterCons.map((con, idx) => (
                                    <li key={idx} className="text-xs text-[#5a4d2a] flex items-start gap-2 leading-relaxed">
                        ...
                        {candidate.status === 'completed' && candidate.screeningScore !== undefined && (
                          <div className="bg-[#f7f6f3] border border-[#e4e1da] rounded-xl p-4 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 bg-[#e8f2ee] rounded-xl flex items-center justify-center">
                                <CheckCircle2 className="w-4.5 h-4.5 text-[#2d6a55]" style={{ width: '18px', height: '18px' }} />
                              </div>
                              <div>
                                <p className="text-sm text-[#1c1c1a]">Screening Complete</p>
                                <p className="text-xs text-[#6b7063]">Interactive warm-up sandbox</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <div className="text-2xl text-[#2d6a55]">{candidate.screeningScore}</div>
                        ...
                        {candidate.recruitmentEmail && (
                          <details className="group bg-white border border-[#e4e1da] rounded-xl overflow-hidden">
                            <summary className="cursor-pointer px-4 py-3 hover:bg-[#f7f6f3] transition-colors flex items-center justify-between">
